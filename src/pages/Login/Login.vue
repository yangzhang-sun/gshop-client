      ...
      <div class="login_content">
        <form>
          ...
          <div :class="{on:isPassWrrldLogin}">
            <section>
              <section class="login_message">
                <input  v-model="phone" v-validate="'required|phone'" type="tel" maxlength="11" name="phone" placeholder="手机号" />
...
export default {
  ...
  methods: {
   ...
   async Login(){
        ...
        if (isPassWordLogin) {
          // 用户名/密码登录
          result = await this.$API.loginWithPassword(phone, pwd, code);
          if (result.code === 1) {
            // 清空图形验证码
            this.captcha = "";
            // 更新图形验证码
          ...
          // 手机号/验证码登录
          result = await this.$API.loginWithPhone(name, captcha);
          if (result.code === 1) {
            // 清空验证码
            this.code = "";
          }
